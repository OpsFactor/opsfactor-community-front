 ...
 */
export async function bootstrapRuntimeInfo(
  expectedEdition: OpsFactorEdition,
  { timeoutMilliseconds = RUNTIME_INFO_TIMEOUT_MILLISECONDS }: RuntimeInfoBootstrapOptions = {},
): Promise<RuntimeInfo> {
  ...
  try {
    runtimeInfo = await runtimeInfoStore.load(
      new HttpClient(() => null),
    ...
    if (bootstrapAbortController.signal.aborted) {
      throw new Error(
        `The ${expectedEdition} runtime did not respond to its public edition check within ${timeoutMilliseconds / 1_000} seconds.`,
      );
    }
  ...
  if (runtimeInfo.edition !== expectedEdition) {
    throw new Error(
      `This ${expectedEdition} frontend requires a ${expectedEdition} backend, but received ${runtimeInfo.edition}.`,
    );
  }
