...
<template>
  <section class="rounded-xl border border-[color:var(--ofx-border)] bg-[color:var(--ofx-surface)] px-5 py-5 shadow-sm">
    ...
    <div v-if="props.metrics?.length" class="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
      <div v-for="metric in props.metrics" :key="metric.label" :class="['min-w-0 rounded-lg border px-4 py-3', toneClasses(metric.tone)]">
        <div class="text-xs uppercase tracking-[0.14em] text-current/75">{{ metric.label }}</div>
        <div class="mt-1 break-words text-sm font-semibold leading-5 text-current">{{ metric.value }}</div>
      </div>
    </div>
  </section>
</template>
