...
<template>
  <TaskPageLayout class="material-master-data-catalog-page">
    ...
    <OfxSectionCard class="catalog-card" aria-labelledby="material-master-data-title">
      <div class="section-heading"><div><p class="eyebrow">Catalog</p><h2 id="material-master-data-title">{{ activeTab === 'materials' ? 'Materials' : 'Material clusters' }}</h2></div><div class="actions"><OfxButton variant="secondary" icon="refresh" v-if="activeTab === 'materials'" type="button" :disabled="loadingScope !== null" @click="void loadMaterials()">{{ isLoadingMaterials ? 'Loading…' : materials === null ? 'Load materials' : 'Reload materials' }}</OfxButton><OfxButton variant="secondary" icon="refresh" v-else type="button" :disabled="loadingScope !== null" @click="void loadMaterialClusters()">{{ isLoadingClusters ? 'Loading…' : materialClusters === null ? 'Load material clusters' : 'Reload material clusters' }}</OfxButton></div></div>
