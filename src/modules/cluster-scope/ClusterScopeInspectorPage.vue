...
<template>
  <DashboardPageLayout class="clustering-page">
    <OfxPageHeader eyebrow="Configuration" title="Clustering" description="Create reusable material and location groups in one planning clustering scheme.">
      <template #actions>
        <OfxButton variant="create" icon="new" type="button" :disabled="isBusy" @click="startCreation">New {{ activeDimension }} cluster</OfxButton>
      </template>
    </OfxPageHeader>
      ...
      <main class="cluster-editor" :aria-busy="isBusy">
        <template v-if="activeDimension === 'material' && materialDraft">
          ...
          <section class="rules-section" aria-labelledby="material-rules-title">
            <div class="rules-heading"><div><h3 id="material-rules-title">Membership rules</h3><p>Rules determine which materials belong to this cluster. Values in one characteristic are alternatives; separate rules are cumulative.</p></div><div class="rule-actions"><OfxButton variant="create" icon="add" type="button" :disabled="saving" @click="addMaterialRule">Add status</OfxButton><OfxButton variant="create" icon="add" type="button" :disabled="saving || materialCharacteristics.length === 0" @click="addMaterialCharacteristicRule">Add characteristic</OfxButton></div></div>
            <article v-for="(rule, index) in materialDraft.regraAlocacaoClusterDTOList" :key="rule.id ?? `new-material-rule-${index}`" class="rule-card">
              ...
              <OfxButton variant="danger" icon="delete" size="compact" type="button" :disabled="saving" @click="removeMaterialRule(index)">Remove</OfxButton>
            </article>
        ...
        <section v-else class="empty-editor"><p class="eyebrow">Cluster definition</p><h2>Start with a cluster</h2><p>Select a cluster or create a new {{ activeDimension }} cluster.</p><OfxButton variant="create" icon="new" type="button" :disabled="isBusy" @click="startCreation">New {{ activeDimension }} cluster</OfxButton></section>

        <footer v-if="activeDraft" class="editor-actions"><OfxButton variant="danger" icon="delete" v-if="activeDraft.id !== null" class="cluster-delete-button" type="button" :disabled="saving" @click="requestClusterDeletion">Delete cluster</OfxButton><OfxButton variant="primary" icon="save" class="save-button" type="button" :disabled="isBusy" @click="void saveActiveDraft()">{{ saving ? 'Saving…' : 'Save cluster' }}</OfxButton></footer>

        <section v-if="activeDimension === 'material' && selectedMaterialCluster" class="members-section"><div><p class="eyebrow">Cluster members</p><h3>Materials in this cluster</h3><p>Members reflect the saved definition. Save changes to update membership.</p></div><p v-if="loadingMaterialClusterMembers" class="snapshot-status">Loading materials…</p><div v-else-if="hasMaterialClusterMembersSnapshot" class="members-table"><OfxDataTable v-if="materialMemberRows.length" :rows="materialMemberRows" :columns="materialMemberColumns" row-key="rowKey" :dense="true" :page-size="10" text-size="xs" export-base-name="cluster-material-members" /><p v-else>No active materials were returned.</p></div><OfxButton variant="secondary" icon="open" v-else type="button" :disabled="isBusy" @click="void loadSelectedMaterialClusterMembers()">Load members</OfxButton></section>
        <section v-if="activeDimension === 'location' && selectedLocationCluster" class="members-section"><div><p class="eyebrow">Cluster members</p><h3>Locations in this cluster</h3><p>Members reflect the saved definition. Save changes to update membership.</p></div><OfxButton variant="secondary" icon="open" v-if="!hasLocationClusterMembersSnapshot" type="button" :disabled="loadingLocationClusterMembers || isBusy" @click="void loadSelectedLocationClusterMembers()">{{ loadingLocationClusterMembers ? 'Loading…' : 'Load members' }}</OfxButton><p v-else class="snapshot-status">Member snapshot loaded.</p><div v-if="hasLocationClusterMembersSnapshot" class="members-table"><OfxDataTable v-if="locationMemberRows.length" :rows="locationMemberRows" :columns="locationMemberColumns" row-key="rowKey" :dense="true" :page-size="10" text-size="xs" export-base-name="cluster-location-members" /><p v-else>No active locations were returned.</p></div></section>
      </main>
    </section>

    <section v-if="pendingDeletion" class="delete-confirmation" role="dialog" aria-modal="true" aria-labelledby="delete-cluster-title"><div><p class="eyebrow">Confirm deletion</p><h2 id="delete-cluster-title">Delete this cluster?</h2><p><strong>{{ pendingDeletion.description }}</strong> will be sent to the server for deletion. Linked plans and records are not removed by this page.</p></div><div><OfxButton variant="secondary" icon="close" type="button" :disabled="deleting" @click="pendingDeletion = null">Keep cluster</OfxButton><OfxButton variant="danger" icon="delete" type="button" :disabled="deleting" @click="void confirmClusterDeletion()">{{ deleting ? 'Deleting…' : 'Delete cluster' }}</OfxButton></div></section>
  </DashboardPageLayout>
</template>
...
@media (max-width: 46rem) { .rules-heading > div:first-child { flex: initial; }.library-selectors { grid-template-columns: minmax(0, 1fr); }.rule-inputs { width: 100%; }.cluster-editor, .cluster-library { padding: 1rem; }.editor-heading, .rules-heading, .rule-card, .delete-confirmation { flex-direction: column; }.definition-fields, .rule-inputs, .location-rule-inputs, .members-section { grid-template-columns: 1fr; }.members-table { grid-column: auto; }.editor-actions button, .rule-card .ofx-button { width: 100%; }.delete-confirmation > div:last-child { width: 100%; }.delete-confirmation > div:last-child button { flex: 1; } }
</style>
