...
<template>
  <TaskPageLayout class="low-level-code-page">
    <OfxPageHeader
      ...
      description="Inspect the raw Community supply dependency path for one explicit material and Supply Network Version."
    />

    <OfxSectionCard class="boundary-card" title="Read-only technical diagnostic">
      <p>The backend resolves the selected material path from existing Community projections. The browser shows the returned nodes and directed edges as tables; it does not create a graph, calculate levels, or infer dependencies.</p>
    ...
    <OfxSectionCard class="selector-card" title="Material path" description="Explicit diagnostic.">
      <div class="field-grid">
        <label>Supply Network Version ID<input v-model="supplyNetworkVersionId" :disabled="loading" type="text"><small>Enter the existing version ID; no catalog is preloaded.</small></label>
        <label>Material ID<input v-model="materialId" :disabled="loading" type="text"><small>Enter the material root; no material catalog is loaded.</small></label>
      </div>
    ...
    <template v-if="lowLevelCodeSnapshot">
      ...
      <OfxSectionCard class="result-card" title="Directed edges" description="Edge direction and labels remain exactly as returned by the Community diagnostic.">
...
<style scoped>
.boundary-card, .selector-card, .result-card { display: grid; gap: 1rem; }.boundary-card p, .empty-state { color: var(--ofx-text-muted); }.field-grid { display: grid; gap: 1rem; grid-template-columns: repeat(auto-fit, minmax(18rem, 1fr)); }.field-grid label { display: grid; gap: .4rem; font-size: .875rem; font-weight: 700; }.field-grid input { border: 1px solid var(--ofx-border); border-radius: .5rem; background: var(--ofx-surface); color: var(--ofx-text); min-height: 2.5rem; padding: .55rem; }.field-grid small { color: var(--ofx-text-muted); font-weight: 400; }.actions { display: flex; justify-content: flex-end; }.primary-button { border: 1px solid var(--ofx-accent); border-radius: .5rem; background: var(--ofx-accent); color: white; cursor: pointer; padding: .65rem .9rem; width: fit-content; }.primary-button:disabled { cursor: not-allowed; opacity: .55; }.table-scroll { overflow-x: auto; }.table-scroll table { border-collapse: collapse; min-width: 52rem; width: 100%; }.table-scroll th, .table-scroll td { border-bottom: 1px solid var(--ofx-border); padding: .7rem; text-align: left; vertical-align: top; white-space: nowrap; }.table-scroll th { color: var(--ofx-text-muted); font-size: .75rem; text-transform: uppercase; }.error { color: var(--ofx-text-danger); }@media (max-width: 48rem) { .actions { justify-content: stretch; }.primary-button { width: 100%; } }
</style>
