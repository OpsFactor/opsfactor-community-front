...
<template>
  <TaskPageLayout class="location-cluster-criteria-catalog-page">
    ...
    <OfxSectionCard class="boundary-card" title="Allowed values, not cluster state">
      ...
      <OfxButton variant="secondary" icon="open" v-if="!hasSnapshot" type="button" :disabled="loading" @click="loadCriteria">{{ loading ? 'Loading catalog…' : 'Load allowed criteria' }}</OfxButton>
